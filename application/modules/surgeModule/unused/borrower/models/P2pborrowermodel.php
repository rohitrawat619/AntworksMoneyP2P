...
//        echo $this->db->last_query(); exit;
    ...
    public function getborrowers_Step_1($pageLimit, $setLimit)
    {
        $this->db->select('BL.id,
                           ...
                           BS.step_2
                          ');
        $this->db->where_in('BS.step_1', array('2', '3'));
        $this->db->from('p2p_borrowers_list AS BL');
        $this->db->join('p2p_borrower_steps AS BS', 'ON BS.borrower_id = BL.id', 'left');
        $this->db->limit($pageLimit, $setLimit);
    ...
    public function action_update_steps()
    {
        ...
        if (isset($_POST['reinitiate_credit_decisioning'])){
            $this->db->where('borrower_id', $this->input->post('borrower_id'));
            $this->db->set('step_2', null);
            $this->db->update('p2p_borrower_steps');
        }

        if (isset($_POST['reinitiate_e_nach'])){
            $this->db->where('borrower_id', $this->input->post('borrower_id'));
            $this->db->set('step_3', null);
            $this->db->update('p2p_borrower_steps');
        }
        ...
        $this->db->insert('p2p_borrower_action_remarks_steps', $log_steps);
    ...
    {
        $query = $this->db->get_where('borrower_basic_filtration_criteria', array('borrower_id' => $borrower_id));
        if ($this->db->affected_rows() > 0) {
            return $query->row_array();
        } else {
            return false;
    ...
    {
        $query = $this->db->order_by('id', 'desc')->get_where('borrower_pan_api_details', array('mobile' => $borrowerId));
        if ($this->db->affected_rows() > 0) {
            return $query->row_array();
        } else {
            return false;
    ...
    {
        $query = $this->db->order_by('id', 'desc')->get_where('ektara_response', array('borrower_id' => $borrowerId));
        if ($this->db->affected_rows() > 0) {
            return $query->row_array();
        } else {
            return false;
    ...
    {
        $query = $this->db->select('bank_account_response AS response')->order_by('id', 'desc')->get_where('p2p_borrower_bank_details', array('mobile' => $borrowerId));
//        pr($this->db->last_query());
    ...
    {
        $query = $this->db->select('razorpay_response_bank_ac AS response')->order_by('id', 'desc')->get_where('p2p_borrower_bank_res', array('mobile' => $borrowerId));
        if ($this->db->affected_rows() > 0) {
            return $query->row()->response;
        } else {
            return false;

            ...
            <div class="row">
                <form method="post" action="<?= base_url('p2padmin/basicfilter/action') ?>"
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <input type="text" required name="min_age" id="min_age" value="<? if ($rule['min_age']) {
                            echo $rule['min_age'];
                        } ?>" class="form-control">
                    </div>
                    <div class="col-md-6 form-group">
                        <level>Maximum AGE</level>
                        <input type="text" required name="max_age" id="max_age" value="<? if ($rule['max_age']) {
                            echo $rule['max_age'];
                        } ?>" class="form-control">
                    </div>
                    <div class="col-md-6 form-group">
                        <level>Salary Less than</level>
                        <input type="text" name="salary_less_than" value="<? if ($rule['salary_less_than']) {
                            echo $rule['salary_less_than'];
                        } ?>" class="form-control" required>
                    </div>
                    <div class="col-md-6 form-group">
                        <level>Minimum Experian Score Less Than</level>
                        <input type="text" name="credit_score" value="<? if ($rule['credit_score']) {
                    ...
                    <div class="col-md-6 form-group">
                        <level>Qualification You want Allow</level>
                            ...
                            foreach ($qualification_list as $q_list) {
                                ...
                                ?>
                                <input type="checkbox" name="qualification[]" value="<?= $q_list['qualification'] ?>" <? if (in_array($q_list['qualification'], $q)) {
                                    echo "checked";
                    ...
                    <div class="col-md-6 form-group">
                        <level>Occupation You want allow</level>
                            ...
                            foreach ($occupation_list as $o_list) { ?>
                                <input type="checkbox" name="occupation[]" value="<?= $o_list['name'] ?>" <? if (in_array($o_list['name'], $o)) {
                                    echo "checked";
                                } ?>> <?= $o_list['name'] ?>
                            <? } ?>
                    </div>
                    <div class="col-md-6 form-group">
                        ...
                        foreach ($company_category_list as $c_list) { ?>
                            <input type="checkbox" name="company_category[]" value="<?= $c_list['company_category'] ?>" <? if (in_array($c_list['company_category'], $c)) {
                                echo "checked";

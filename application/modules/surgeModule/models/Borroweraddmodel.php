...
class Borroweraddmodel extends CI_Model
{
    ...
    public function pan_basis($borrower_id, $mobile, $pan, $name, $name_match_method)
    {
		...
		if ($this->cldb->affected_rows() > 0)
        {
            ...
            if ($borrower_id) {
                $this->cldb->get_where('p2p_borrower_steps', array('borrower_id' => $borrower_id));
				
                if ($this->cldb->affected_rows() > 0) {
                    if ($arr_response['result']['name_match'] == true) {
                        $this->cldb->where('borrower_id', $borrower_id);
                        $this->cldb->update('p2p_borrower_steps', array('step_1' => 1));
                    } else {
                        if ($arr_response['result']['status'] == 'Invalid') {
                            $this->cldb->where('borrower_id', $borrower_id);
                            $this->cldb->update('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 3));
                        } else if ($arr_response['result']['name_match'] == false) {
                            $this->cldb->where('borrower_id', $borrower_id);
                            $this->cldb->update('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 2));
                        }
                    }
                } else {
                    if ($arr_response['result']['name_match'] == true) {
                        $this->cldb->insert('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 1));
                    } else {
                        if ($arr_response['result']['status'] == 'Invalid') {

                            $this->cldb->insert('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 3));
                        } else if ($arr_response['result']['name_match'] == false) {
                            $this->cldb->insert('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 2));
                        }
                    }
                }
            }
    ...
    public function basic_filteration($dob, $pan, $pincode, $qualification, $occuption_type, $company_code, $income, $credit_score)
    {
        ...
        $kyc_status = 1;
        $kyc_status_msg = '';
        $query = $this->cldb->order_by('id', 'desc')->get_where('p2p_borrower_steps', array('borrower_id' => $this->input->post('borrower_id')));
        if ($this->cldb->affected_rows() > 0) {
...
file_put_contents('bankkyc', date('Y-m-D H:i:s') .$this->cldb->last_query().'==='. $this->input->post('user_type').PHP_EOL, FILE_APPEND);      
   ...
   public function add_bank_detail_borrower_lender()
    ...
    public function loan_eligibility_status()
    {
        ...
        if ($this->cldb->affected_rows() > 0) {
            $query = $this->cldb->get_where('p2p_borrower_steps', array('borrower_id' => $this->input->post('borrower_id')));

    ...
    $(document).ready(function() {
        ...
        $('.otp-form').submit(function(e) {
		 ...
		 $('#loading-message').show();
            ...
            $.ajax({
                url: 'e_sign_verify_lender_agreement_otp', // Replace with the actual PHP file handling OTP verification
                type: 'POST',
                data: {
                    otp: otp
                },
                success: function(response) {
					...
					console.log(resp)
                    if (resp.status == 1) {
                        window.location.href = 'processingInvestmentPayment?q=<?php echo $encryptedInvestSchemeNAmount; ?>';
                    } else {

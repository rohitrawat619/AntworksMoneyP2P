<?php
class Invest_model_p2p extends CI_Model{
		...
		if($this->db->affected_rows()>0)
        {
			...
			if($lenderDetails->pan == $this->input->post('PAN')){
				...
				$postData = array(
						...
						'source_of_lead' => $this->input->post('source'),
						'created_date'=>date("Y-m-d H:i:s"),
						'modified_date'=>date("Y-m-d H:i:s"),
					);
				
			     $this->db->insert('p2p_lender_list',$postData);
					if($this->db->affected_rows()>0)
                     {
    ...
    {
        $query = $this->db->select('*')->get_where('invest_scheme_details',array('status'=>1,'Vendor_ID'=>$this->input->post('vendor_id')));
	...
	public function getKycStatus($mobile)
    {
        ...
        $query = $this->db->select('PLL.lender_id, PLL.vendor_id, kyc.kyc_api_status, PLI.ant_txn_id, PLI.investment_No')
		->join('p2p_lender_investment as PLI', 'PLI.mobile = PLL.mobile', 'left')
		->join('all_kyc_api_log as kyc', 'PLI.mobile = kyc.mobile', 'left')
    ...
    {
        $this->db->select("investment_No");
        $this->db->from('p2p_lender_investment');
        $this->db->order_by('investment_No', 'DESC');
        $this->db->limit(1);
        $query = $this->db->get();
        $row = (array)$query->row();
        if($this->db->affected_rows()>0)
	...
	public function investment_details()
    {
		...
		$invested_amount = 0;
        $current_value = 0;
        $query = $this->db->select('ISD.Scheme_Name, ISD.Lockin, ISD.Lockin_Period, ISD.Cooling_Period, LI.investment_No ,LI.lender_id, LI.mobile, LI.scheme_id, LI.amount, LI.basic_rate, LI.hike_rate, LI.pre_mat_rate, LI.redemption_date, LI.redemption_status, LI.created_date as investment_date')
		->join('invest_scheme_details as ISD', 'ISD.id = LI.scheme_id', 'left')
		->order_by('LI.created_date','DESC')
		->get_where('p2p_lender_investment as LI',array('LI.mobile'=>$postVal['phone'], 'LI.redemption_status'=>0));
		//echo $this->db->last_query();exit;
        if($this->db->affected_rows()>0)
        {
			...
			$total_current_value = 0;
				foreach($result as $row)
				 ...
				 #Update Final Interest and current Value
				   $this->db->where('investment_No', $row['investment_No']);
				   $this->db->where('mobile', $postVal['phone']);
				   $this->db->update('p2p_lender_investment',array(
	...
	public function update_investment_cron()
    {
		...
		->get_where('p2p_lender_investment as LI',array('LI.source'=>'surge','LI.redemption_status'=>0));
	...
	public function redemption_request()
    {
        ...
        $query = $this->db->select('ISD.Scheme_Name, ISD.Lockin, ISD.Lockin_Period, ISD.Cooling_Period, LI.investment_No ,LI.lender_id, LI.mobile, LI.scheme_id, LI.amount, LI.basic_rate, LI.hike_rate, LI.pre_mat_rate, LI.redemption_date, LI.redemption_status, LI.created_date as invest_date, bank.razorpay_response_fav')
		->join('invest_scheme_details as ISD', 'ISD.id = LI.scheme_id', 'left')
		->join('p2p_borrower_bank_res as bank', 'bank.mobile = LI.mobile', 'left')
		->get_where('p2p_lender_investment as LI',array('LI.mobile'=>$postVal['phone'],'LI.source'=>$postVal['source'],'LI.investment_No'=>$postVal['investment_no'],'bank.fav_id !='=> ''));
		//echo $this->db->last_query();exit;
        if($this->db->affected_rows()>0)
        {
            $result = $query->row();
    ...
    {
        $query = $this->db->select('PLL.lender_id, PLL.vendor_id, PLL.pan_kyc, PLL.account_kyc, PLI.ant_txn_id, PLI.investment_No')
		->join('p2p_lender_investment as PLI', 'PLI.mobile = PLL.mobile', 'left')
		->order_by('PLI.reinvestment_id', 'DESC')
		->get_where('p2p_lender_list PLL', array('PLL.mobile' => $mobile));
		//echo $this->db->last_query();exit;
...
public function basic_bank_kyc($details) {
   	 ...
   	 public function addaccount($response,$lender_details)
    {
        ...
        if($this->db->affected_rows()>0)
        {
		$query = $this->db->get_where('p2p_lender_account_info',array('lender_id' => $lender_details->user_id));
		$bankdetails = array(
                'lender_id' => $lender_details->user_id,
		...
		if($this->db->affected_rows()>0)
        {
			...
			$this->db->where('lender_id', $lender_details->user_id);
			$this->db->update('p2p_lender_account_info', $bankdetails);
		}else{
	...
	public function update_lender_id_in_pan_kyc_table($mobile,$pan,$lenderID){
		...
		$this->db->where('source', 'surge');
		$this->db->where('pan', $pan);
		$this->db->update('borrower_pan_api_details', array('lender_id' => $lenderID));
		//echo $this->db->last_query();exit;
		return true;
	}

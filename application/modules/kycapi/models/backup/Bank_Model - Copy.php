...
#Old functions use in credit-line Borrower model
    ...
    public function registration_credit_line_1()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            ####
            $this->db->select('borrower_id')->get_where('p2p_borrower_address_details', array('borrower_id' => $last_insert_borrower_id));
            if ($this->db->affected_rows() > 0) {
                ...
                $this->db->where('borrower_id', $last_insert_borrower_id);
    ...
    {
        $query = $this->db->get_where('p2p_borrower_steps', array('borrower_id' => $borrower_id));
        if ($this->db->affected_rows() > 0) {
    ...
    {
        $query = $this->db->get_where('borrower_pan_api_details', array('pan' => $this->input->post('pan'), 'name' => $name));
		if ($this->db->affected_rows() > 0)
        {
            ...
            if ($borrower_id) {
                $this->db->get_where('p2p_borrower_steps', array('borrower_id' => $borrower_id));
                if ($this->db->affected_rows() > 0) {
                    if ($arr_response['result']['name_match'] == true) {
                        $this->db->where('borrower_id', $borrower_id);
                        $this->db->update('p2p_borrower_steps', array('step_1' => 1));
                    } else {
                        if ($arr_response['result']['status'] == 'Invalid') {
                            $this->db->where('borrower_id', $borrower_id);
                            $this->db->update('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 3));
                        } else if ($arr_response['result']['name_match'] == false) {
                            $this->db->where('borrower_id', $borrower_id);
                            $this->db->update('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 2));
                        }
                    }
                } else {
                    if ($arr_response['result']['name_match'] == true) {
                        $this->db->insert('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 1));
                    } else {
                        if ($arr_response['result']['status'] == 'Invalid') {

                            $this->db->insert('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 3));
                        } else if ($arr_response['result']['name_match'] == false) {
                            $this->db->insert('p2p_borrower_steps', array('borrower_id' => $borrower_id, 'step_1' => 2));
                        }
                    }
                }
            }
       ...
       curl_setopt_array($curl, array(
        ...
        /* $this->db->insert('borrower_pan_api_details', array(
            'borrower_id' => $borrower_id,
            'mobile' => $this->input->post('mobile'),
            'pan' => $this->input->post('pan'),
    ...
    public function basic_filteration($dob, $pan, $pincode, $qualification, $occuption_type, $company_code, $income, $credit_score)
    {
        ...
        $kyc_status = 1;
        $kyc_status_msg = '';
        $query = $this->db->order_by('id', 'desc')->get_where('p2p_borrower_steps', array('borrower_id' => $this->input->post('borrower_id')));
        if ($this->db->affected_rows() > 0) {
        ...
        /*$query = $this->db->order_by('id', 'desc')->get_where('borrower_pan_api_details', array('borrower_id' => $this->input->post('borrower_id')));
        if ($this->db->affected_rows() > 0) {
    ...
    {
        $query = $this->db->get_where('p2p_borrower_bank_res', array('mobile' => $this->input->post('mobile'), 'account_no' => $this->input->post('account_no'), 'ifsc_code' => $this->input->post('ifsc_code'), 'bank_registered_name is not null' => null));
file_put_contents('bankkyc', date('Y-m-D H:i:s') .$this->db->last_query().'==='. $this->input->post('user_type').PHP_EOL, FILE_APPEND);      
	  if ($this->db->affected_rows() > 0)
        {
            $bank_result = $query->row_array();
            ...
            if ($this->input->post('borrower_id')) {
                ...
                $this->db->get_where('p2p_borrower_bank_details', array('borrower_id' => $this->input->post('borrower_id')));
                if ($this->db->affected_rows() > 0) {
                    $this->db->where('borrower_id', $this->input->post('borrower_id'));
                    $this->db->update('p2p_borrower_bank_details', $bank_data);
                } else {
    ...
    public function loan_eligibility_status()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $query = $this->db->get_where('p2p_borrower_steps', array('borrower_id' => $this->input->post('borrower_id')));
    ...
    public function ektara_first_api($borrowerId)
    {
		...
		//file_put_contents('log', date('Y-m-D H:i:s') . json_encode($borr_info).PHP_EOL, FILE_APPEND);
        $curl = curl_init();
        curl_setopt_array($curl, array(

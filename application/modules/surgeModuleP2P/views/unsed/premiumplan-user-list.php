						...
						<div class="col-md-4 form-group">
							...
							<select class='form-control' name='status' id='status'>
								<option value=''>-- Select Lead Status --</option>
									<?php if ($user_list['status'] != "15"){?>
								<option value='4' <?php if ($user_list['status'] == "4") {
									echo "selected";
								} ?>>Language Barrier
								</option>
								<option value='6' <?php if ($user_list['status'] == "6") {
									echo "selected";
								} ?>>Reminder
								</option>
								<option value='2' <?php if ($user_list['status'] == "2") {
									echo "selected";
								} ?>>Process
								</option>
								<option value='3' <?php if ($user_list['status'] == "3") {
									echo "selected";
								} ?>>Phone Not Picked
								</option>
								<option value='7' <?php if ($user_list['status'] == "7") {
									echo "selected";
								} ?>>Not Interested
								</option>
								<option value='5' <?php if ($user_list['status'] == "5") {
									echo "selected";
								} ?>>Duplicate
								</option>
								
								<option value='14' <?php if ($user_list['status'] == "14") {
									echo "selected";
								} ?>>P2P registration done
								</option>
								<?php } ?>
								<option value='15'>P2P Payment done
								</option>
...
<button id="<?php echo $this->session->userdata('user_id'); ?>button"  type="button" class="btn btn-danger" onclick='getCreditScoreApiCall("<?php echo $user_list['mobile']; ?>","<?php echo $user_list['fname']; ?>","<?php echo $user_list['lname']; ?>","free","<?php echo $this->session->userdata('user_id'); ?>","ppJS")' >Show Score</button>
							...
							<!---------starting of sms func----------->
						<div class="col-md-4 form-group">
							<input class="form-control" placeholder="Enter SMS Text" type="text"
								   id="smsMessage" >
								   <button type="button" onclick="send_panel_sms()" class=" form-control btn btn-success" >Send SMS</button>
						</div>
								<!-------------ending of sms func---------->
				...
				<div class="row">
					<div class="box-footer text-right">
						...
						<input type="button" name="submit" id="saveremark" value="Update User" class="btn btn-success"/>
					</div>
				</div>

...
class P2pborrower extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('P2pborrowermodel');
        error_reporting(0);
    }
...
//        echo "<pre>";
//        var_dump($data['list']['is_bank_verified']);
...
//        echo $this->db->last_query(); exit;
                if ($this->db->affected_rows() > 0) {
                    ...
                    $delimiter = ",";
                    $newline = "\r\n";
                    $filename = "BorrowerExperiandetails.csv";
                    $data = $this->dbutil->csv_from_result($query, $delimiter, $newline);
                    force_download($filename, $data);
                }
...
exit;
                $this->db->select("*");
                $this->db->from('p2p_borrower_occuption_details');
                $this->db->order_by('id', 'desc');
                $query = $this->db->get();
//        echo "<pre>";
//        echo $this->db->last_query(); exit;

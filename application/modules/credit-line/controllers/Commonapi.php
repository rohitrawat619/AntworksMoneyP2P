...
class Commonapi extends REST_Controller
{
    ...
    {
        $results = $this->db->get_where('p2p_list_company')->result_array();
        $response = array(
            'status' => 1,
	...
	public function companylist_post()
    {
       ...
       $results = $this->db->select('*')->from('p2p_list_company')
           ->like('company_name', $keyword)->get()->result_array();	
        $response = array(
            'status' => 1,
            'company_list' => $results,
            'msg' => 'found',
    ...
    {
        $response = array('company_type' => ["Government", "PSUs", "MNC", "Private Limited Company", "Public Limited Company", "Others"]);
        $this->set_response($response, REST_Controller::HTTP_OK);
        return;
    }

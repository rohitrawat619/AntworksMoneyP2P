...
class Borrowerloan extends REST_Controller
{
    public function __construct($config = 'rest')
    {
        ...
        $this->load->model(array('Borrowermodel', 'Borroweractivitymodel', 'Borrowerinfomodel', 'bidding/Biddingmodel'));
        $this->load->database();
    }
   ...
   public function viewLoanaggrement_post()
    {
        error_reporting(0);
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $result = $this->Loanaggrementmodel->loanaGGrementCreditLine();
	...
	public function generate_order_repayment_post()
    {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $res = $this->Ordermodel->generateOrder();
                if ($res) {
	...
	public function loan_details_post()
	{
		...
		if ($auth) { */
			...
			$this->form_validation->set_rules('borrower_id', 'borrower_id', 'required');
			if ($this->form_validation->run() == TRUE) {
				$res = $this->Borrowerinfomodel->loan_details();
				if ($res) {
					$this->set_response($res, REST_Controller::HTTP_OK);
					return;
				} else {
	...
	public function loan_after_payment_post()
	{
				$_POST = json_decode(file_get_contents('php://input'), true);
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($this->input->post('payment_method') == 'PG') {
						...
						if ($this->form_validation->run() == TRUE) {
							...
							$verify = $this->Requestmodel->verifiedRazorpaypayment_bill();
							if ($verify === true) {
								$response = $response = array('status' => 1);
								$this->set_response($response, REST_Controller::HTTP_OK);
								return;
							} else {
								$this->set_response("Invalid Payment Response", REST_Controller::HTTP_UNAUTHORIZED);
								return;
							}
						}
					}
					/* if ($this->input->post('payment_method') == 'UPI') {
						...
						$this->form_validation->set_rules('statusDesc', 'statusDesc', 'required');
						if ($this->input->post('statusCode') == 'S') {
							$response = $response = array('status' => 1);
							$this->set_response($response, REST_Controller::HTTP_OK);
							return;
						} else {
	...
	public function credit_line_sendOtpsignature_post()
    {
        /* $auth = $this->middleware->auth();
        if ($auth) { */
            $_POST = json_decode(file_get_contents('php://input'), true);
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $borrowerInfo = $this->Borrowerinfomodel->get_personalDetails($this->input->post('borrower_id'));
     ...
     */
	 ...
	 public function credit_line_verifyOtpsignature_post()
    {
        ...
        if ($auth) { */
            ...
            if ($this->form_validation->run() == TRUE) {
                $borrowerId = $this->input->post('borrower_id');
	...
	public function credit_line_disbursement_request()
	{

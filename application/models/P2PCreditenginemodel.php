...
class P2PCreditenginemodel extends CI_Model
{
	...
	public function Engine($borrower_id)
	{
		$this->load->database();
		error_reporting(0);
		$this->db->select('BL.id, BL.name, BL.highest_qualification, BL.occuption_id, TIMESTAMPDIFF(YEAR, BL.dob, CURDATE()) AS age,
                           ADDR.r_city,
                           ADDR.r_pincode,
		...
		if ($this->db->affected_rows() > 0) {
            ...
            if($result['flag'] == 1)
			{
				$removesoap_tag = str_replace('<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/"><SOAP-ENV:Header/><SOAP-ENV:Body><ns2:processResponse xmlns:ns2="urn:cbv2"><ns2:out>', '', $file_content);
				$experian_response = str_replace('</ns2:out></ns2:processResponse></SOAP-ENV:Body></SOAP-ENV:Envelope>', '', $removesoap_tag);
			}
            else{
				...
				if (!empty($experian_score)) {
					...
					if (in_array($no_record_found, $message_code)) {
						...
						if ($experian_score > 300) {
							...
							for ($x = 0; $x < $cais_account_total; $x++) {
								$address_count = count($report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details']);
								if (is_array($report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details'][0])) {
									$address1 = '';
									$address2 = '';
									$address_count = count($report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details']);
									for ($y = 0; $y < $address_count; $y++) {
										$address1 .= $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details'][$y]['First_Line_Of_Address_non_normalized'];
										$address2 .= $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details'][$y]['Second_Line_Of_Address_non_normalized'] ? $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details'][$y]['Second_Line_Of_Address_non_normalized'] : '';

									}
									$address_list[] = $address1 . $address2;
								} else {
									$address1 = $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details']['First_Line_Of_Address_non_normalized'];
									$address2 = $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details']['Second_Line_Of_Address_non_normalized'] ? $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Address_Details']['Second_Line_Of_Address_non_normalized'] : '';
									$address_list[] = $address1 . $address2;
								}
							}
							$phone_list = array();
							for ($x = 0; $x < $cais_account_total; $x++) {
//                                echo count($report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Phone_Details']);
//                                var_dump($report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Phone_Details']);
//                                exit;
								if ($report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Phone_Details']['Mobile_Telephone_Number']) {
									$phone_list[] = $report['CAIS_Account']['CAIS_Account_DETAILS'][$x]['CAIS_Holder_Phone_Details']['Mobile_Telephone_Number'];
								}
							}
							return $data['rating_ratio'] = array(
								'experian_user_first_name' => $report['Current_Application']['Current_Application_Details']['Current_Applicant_Details']['First_Name'],
								'experian_user_last_name' => $report['Current_Application']['Current_Application_Details']['Current_Applicant_Details']['Last_Name'],
								...
								'applications_address_list' => $address_list,

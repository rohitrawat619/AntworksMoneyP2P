<section class="hidden-sidebar collapse" id="sidebarCollapse">
	<button class="close-button" type="button" data-toggle="collapse" data-target="#sidebarCollapse" aria-expanded="false" aria-controls="sidebarCollapse"><i class="fa fa-times"></i></button>
	<div class="single-hidden-sidebar logo">
		<a href="<?=base_url();?>"><img src="<?=base_url();?>assets/img/logo.png" alt="Awesome Image"/></a>
	</div><!-- /.single-hiddeen-sidebar -->
	<div class="single-hidden-sidebar">
		<h3>About Us</h3>
		<p>ANTWORKSMONEY.COM has been setup by ANTWORKS CAPITAL, registered under Indian Companies Act (LLPIN AAB0157). Our principal offices are in Gurgaon and Faridabad. We have plans to open offices across India in near future.</p>
		<a href="#" data-toggle="modal" data-target="#exampleModal-1" class="thm-btn">Consultation</a>
	</div><!-- /.single-hidden-sidebar -->
	<div class="single-hidden-sidebar">
		<h3>Contact Info</h3>
		<ul class="contact-info">
			<li>
				<div class="icon-box">
					<i class="fa fa-map-marker"></i>
				</div><!-- /.icon-box -->
				<div class="text-box">
					<p>UL-03, Eros EF3 Mal, Sector-20 A,
Mathura Road, Faridabad,121001, Haryana</p>
				</div><!-- /.text-box -->
			</li>
			<li>
				<div class="icon-box">
					<i class="fa fa-phone"></i>
				</div><!-- /.icon-box -->
				<div class="text-box">
					<p>8882750000</p>
				</div><!-- /.text-box -->
			</li>
			<li>
				<div class="icon-box">
					<i class="fa fa-envelope-o"></i>
				</div><!-- /.icon-box -->
				<div class="text-box">
					<p>support@antworksmoney.com</p>
				</div><!-- /.text-box -->
			</li>
			<!--<li>
				<div class="icon-box">
					<i class="fa fa-clock-o"></i>
				</div>
				<div class="text-box">
					<p>Week Days: 09.00 to 18.00 <br />Sunday: Closed</p>
				</div>
			</li>-->
		</ul>
	</div><!-- /.single-hidden-sidebar -->
</section>
<?php
class Grievance_redressal extends CI_Controller{
    ...
    public function index()
    {
        ...
        $this->load->view('frontend/common-pages/grievance-redressal-policy',$data);
        $this->load->view('templates/footer');
    }

    public function Grievance_redressal_form() {
        ...
        if ($err) {
            ...
            if ($data['success'] == 1) {
                if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['mobile']) && !empty($_POST['subject']) && !empty($_POST['message'])) {
                    ...
                    if($result)
                    {
                        redirect(base_url().'grievance-redressal/thankyou');
                    }
                    else{
                        redirect(base_url().'grievance-redressal');
                    }
                }
                else{
    ...
    public function thankyou()
    {
        ...
        $this->load->view('templates/nav',$data);
        $this->load->view('templates/collapse-nav',$data);
        $this->load->view('frontend/common-pages/grievance-thankyou',$data);
        $this->load->view('templates/footer');
    }
}
?>